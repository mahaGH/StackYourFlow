		...
		<div id="list-vote" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<th><g:message code="vote.refTo.label" default="Ref To" /></th>
					
						<g:sortableColumn property="value" title="${message(code: 'vote.value.label', default: 'Value')}" />
					
						<th><g:message code="vote.voter.label" default="Voter" /></th>
				...
				<g:each in="${voteInstanceList}" status="i" var="voteInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${voteInstance.id}">${fieldValue(bean: voteInstance, field: "refTo")}</g:link></td>
					
						<td>${fieldValue(bean: voteInstance, field: "value")}</td>
					
						<td>${fieldValue(bean: voteInstance, field: "voter")}</td>
